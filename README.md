...
<details>
<summary>1. 삼성전자 네트워크사업부 | PCB 설계 Web CAD 프로그램 개발 <b>(팀장 / FE Architect)</b></summary>

<br>

`2025.08 ~ 2025.11` `VanillaJS` `OOP` `SRP` `SSoT`

3개월간, 디지털시티에서 근무하며 약 400명의 연구원을 위해 수작업으로 2~3일이 소요되던 PCB 설계 프로세스를 자동화하는 웹 플랫폼을 개발했습니다.

- **문제 정의**: 부서마다 다른 Tool(Excel, Visio 등)을 사용해 데이터를 변환하고 취합하는 과정에서 발생하는 병목 현상이 핵심 문제였습니다. 디지털 스레드를 구축하여 디지털 트윈 사업에 기여하기 위해 단순 명세서를 넘어 현업 담당자 10여 명을 직접 인터뷰하며 숨겨진 Pain-Point를 파악하고, 이를 기반으로 전체 아키텍처와 핵심 기능을 설계했습니다.
- **기술적 기여**:
  - **아키텍처 설계**:
    - VanillaJS : 향후 유지보수성과 확장성을 최우선으로 고려하여, 특정 프레임워크에 대한 의존성을 제거하고자 Vanilla JS를 채택했습니다.
    - OOP / SRP : 객체지향(OOP) 및 단일 책임 원칙(SRP)을 기반으로 FE 아키텍처를 설계하여 안정성과 확장성을 확보했습니다.
    - SSoT : 프론트엔드 상태관리를 위해 진실된 단일 원천 데이터 소스와, 단방향 통신 로직을 구현했습니다.
    - Event : 커스텀 이벤트 기반 아키텍처로 FE 모듈들을 연결하여, 모듈 간 결합도를 크게 낮추었습니다.
  - **역할**:
    - 아키텍처 설계
    - Drawing Module 개발 (Konva.js)
    - 이벤트로 모듈들을 연결
- **기대 효과**:
  - 디지털 스레드 구축 : 데이터 축적 파이프라인을 통해 향후 디지털 트윈 및 AI 도입의 기반을 마련. 
...
<details>
<summary>2. PicTale - AI 인터랙티브 그림책 제작 플랫폼 <b>(팀장 / UX 설계 / FE 총괄)</b></summary>
